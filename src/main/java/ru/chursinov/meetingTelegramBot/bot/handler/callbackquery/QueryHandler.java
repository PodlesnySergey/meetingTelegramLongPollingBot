...
public class QueryHandler implements CallbackQueryHandler{
    ...
    @Autowired
    public QueryHandler(ReplyMessageService replyMessageService, BotConditionUserContext botConditionUserContext, SaveAnswers saveAnswers, UserDataCache userDataCache) {
    ...
    public PartialBotApiMethod<? extends Serializable> handleCallbackQuery(CallbackQuery callbackQuery) {
        ...
        switch (callBackData) {
            case "YES":
                ...
                userProfileData = userDataCache.getUserProfileData(chatId);
                userProfileData.setProblem("Да");
                userDataCache.saveUserProfileData(chatId, userProfileData);
